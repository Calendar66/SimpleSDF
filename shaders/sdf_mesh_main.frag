...
layout(binding = 0) uniform SDFCornellUniforms {
    ...
    vec4 meshHalfSize;      // SDF mesh half extents (xyz)
...
// Flower texture
// Removed flower texture; no longer used after replacing spheres with a box
...
int getMaterial(vec3 p) {
    ...
    if (ground == minDist) return 2;
    if (ceiling == minDist) return 3;
    if (leftWall == minDist) return 4;
    if (rightWall == minDist) return 5;
    if (backWall == minDist) return 6;
    return 2;
}
...
vec3 getLight(vec3 p, vec3 n, vec3 viewDir, vec3 albedo, int matId) {
    ...
    // Override with material-specific values for盒子
    if (matId == 1) {
