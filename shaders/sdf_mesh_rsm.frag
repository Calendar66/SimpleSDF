...
layout(binding = 0) uniform SDFCornellUniforms {
    ...
    vec4  indirectParams;     // 间接光参数
    vec4  debugParams;        // 调试参数 (x=showRSMOnly)
    // PBR parameters and base color factors (to match CPU UBO layout)
    vec4  pbrParams;          // x=enablePBR, y=roughness, z=metallic
    vec2  roughnessValues;    // per-material roughness
    vec2  metallicValues;     // per-material metallic
    vec4  baseColorFactors;   // RGB + intensity
    // SDF mesh mapping params
    vec4  meshHalfSize;       // xyz half extents of bounding box
    vec4  sdfParams0;         // x=minScale, y=hasTex
} u;
// SDF mesh 3D texture
...
float sceneSDF(vec3 p) {
    float meshShape = sdfMeshShape(p);
...
vec3 getMaterialAlbedo(vec3 p) {
    ...
    walls = min(walls, ground);
    if (meshShape < walls) {
        return u.sphereColor.rgb; // mesh albedo
    }
    float minDist = min(min(min(min(ground, leftWall), rightWall), backWall), ceiling);
    if (ground == minDist)    return vec3(0.3, 0.4, 0.6);
    if (ceiling == minDist)   return vec3(0.7, 0.8, 0.9);
    if (leftWall == minDist)  return vec3(0.4, 0.7, 0.5);
    if (rightWall == minDist) return vec3(0.7, 0.4, 0.4);
    if (backWall == minDist)  return vec3(0.6, 0.4, 0.7);
    return vec3(0.8);
}
